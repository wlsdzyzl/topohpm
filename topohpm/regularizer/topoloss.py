...
def getCriticalPoints(likelihood, dims = [1]):
    """
    ...
    res = []
    for dim in dims:
        if (len(pairs_lh[0])<=dim): 
            res.append((0, 0, 0, False))
        else:
            # return persistence diagram, birth/death critical points
            pd_lh = numpy.array([[lh_vector[pairs_lh[0][dim][i][0]], lh_vector[pairs_lh[0][dim][i][1]]] for i in range(len(pairs_lh[0][dim]))])
            bcp_lh = numpy.array([[pairs_lh[0][dim][i][0]//lh.shape[1], pairs_lh[0][dim][i][0]%lh.shape[1]] for i in range(len(pairs_lh[0][dim]))])
            dcp_lh = numpy.array([[pairs_lh[0][dim][i][1]//lh.shape[1], pairs_lh[0][dim][i][1]%lh.shape[1]] for i in range(len(pairs_lh[0][dim]))])
            res.append((pd_lh, bcp_lh, dcp_lh, True))
    return res

def getTopoLoss(likelihood_tensor, gt_tensor, topo_size=100, dims = [1]):
    """
    ...
    for y in range(0, likelihood.shape[0], topo_size):
        for x in range(0, likelihood.shape[1], topo_size):
            ...
            res_lh = getCriticalPoints(lh_patch, dims)
            res_gt = getCriticalPoints(gt_patch, dims)
            for dim_idx in range(len(dims)):
                pd_lh, bcp_lh, dcp_lh, pairs_lh_pa = res_lh[dim_idx]
                pd_gt, bcp_gt, dcp_gt, pairs_lh_gt = res_gt[dim_idx]
    ...
    loss = torch.Tensor([0.0]).type(likelihood_tensor.dtype).to(likelihood_tensor.device)
    # if topo_cp_weight_map.sum() > 0:
    #     loss += (((likelihood_tensor * topo_cp_weight_map) - topo_cp_ref_map) ** 2).sum() / topo_cp_weight_map.sum()
    # print(topo_cp_weight_map.sum())
    loss += (((likelihood_tensor * topo_cp_weight_map) - topo_cp_ref_map) ** 2).sum()
    return loss    

# currently, only for 2D image.
class TopoLoss(nn.Module):
    def __init__(self, dims, normalization = 'Sigmoid',topo_patch_size = 100, **kwargs):
    ...
    def forward(self, pred, gt, weight = None):
        ...
        if weight is not None and weight.shape == pred.shape:
            pred = pred * weight
            gt = gt * weight
        pred = pred.squeeze(0).squeeze(0).squeeze(0)
        gt = gt.squeeze(0).squeeze(0).squeeze(0)
        return getTopoLoss(pred, gt, self.topo_patch_size, self.dims)