...
from topohpm.unet3d.losses import compute_per_channel_dice
from topohpm.unet3d.seg_metrics import AveragePrecision, Accuracy
from topohpm.unet3d.utils import get_logger, expand_as_one_hot, convert_to_numpy
...
class MeanIoU:
    ...
    def __init__(self, skip_channels=(), ignore_index=None, threshold = 0.5, **kwargs):
        """
...
class AdjustedRandIndex:
    ...
    def __init__(self, threshold = 0.5, **kwargs):
        self.threshold = threshold
    def __call__(self, input, target):
        pred_label = (input > self.threshold).astype(int)
        target = target.astype(int)
        return torch.tensor(adjusted_rand_score(pred_label.flatten(), target.flatten()))
...
class AdaptedRandError:
    ...
    def __init__(self, use_last_target=False, ignore_index=None, threshold = 0.5, **kwargs):
...
def get_evaluation_metric(config):
    ...
    def _metric_class(class_name):
        m = importlib.import_module('topohpm.unet3d.metrics')
