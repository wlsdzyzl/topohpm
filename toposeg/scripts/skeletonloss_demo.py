...
pred_path = "/home/*****/project/toposeg/toposeg/scripts/skeleton_loss_demo/pred.png"
label_path = "/home/*****/project/toposeg/toposeg/scripts/skeleton_loss_demo/gt.png"

output_dir = '/home/*****/project/toposeg/toposeg/scripts/skeleton_loss_demo'
...
def detect_critical_points_gudhi(likelihood, label, dims = [0]):
    ...
    likelihood[ np.logical_and(likelihood < 0.4, label < 0.5 )] = 0.0
...
for tick in ax.yaxis.get_major_ticks():
    ...
    tick.label2.set_visible(False)
dpi=200
ax.scatter(birth_ids[0][:, 0], birth_ids[0][:, 1], label='birth points', zorder=10, s=100)
ax.scatter(death_ids[0][:, 0], death_ids[0][:, 1], marker="^", label = 'death points', zorder=10, s=100)
ax.legend()
fig.savefig(fname = output_dir + '/final_grids_cl.png', dpi=dpi)


## generate raw data
def generate_raw_data(pred, label, positive = [144, 255], negative = [32, 150], noise_mean = [-1, 1], noise_std = [-15, 30]):
