...
from toposeg.unet3d.utils import expand_as_one_hot
...
class BCEDCLDiceLoss(nn.Module):
    def __init__(self, alpha, beta, gamma, normalization = 'sigmoid', cl_iter = 10):
        super(BCEDCLDiceLoss, self).__init__()
        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma
        self.dice = DiceLoss(normalization = 'none')
        self.cldice = CLDiceLoss(normalization = 'none', iter_ = cl_iter)
        self.bce = nn.BCELoss()
        assert normalization in ['sigmoid', 'softmax', 'none']
        if normalization == 'sigmoid':
            self.normalization = nn.Sigmoid()
    ...
    def forward(self, input, target):
        input = self.normalization(input)
        # print(input.shape, target.shape, input_cl.shape, target_cl.shape)
        return self.alpha * self.bce(input, target) + self.beta * self.dice(input, target) + self.gamma * self.cldice(input, target)
...
#     def forward(self, input, target):
#         # if weight is not None and weight.shape == input.shape:
#         #     input = input * weight
...
#     def __init__(self, alpha, beta, alpha_focal, gamma):
#         super(FocalDiceLoss, self).__init__()
#         self.alpha = alpha
#         self.beta = beta
#         self.focal = FocalLoss(alpha = alpha_focal, gamma = gamma, reduction = 'mean')
#         self.dice = DiceLoss()

#     def forward(self, input, target):
#         return self.alpha * self.focal(input, target) + self.beta * self.dice(input, target)
...
def _create_loss(name, loss_config, weight, ignore_index, pos_weight):
    if name == 'BCEWithLogitsLoss':
        ...
        iter_ = loss_config.get('cl_iter', 10)
        return BCEDCLDiceLoss(alpha = alpha, beta = beta, gamma = gamma, cl_iter = iter_)      
    # elif name == "FocalDiceLoss":
    #     alpha = loss_config.get('alpha', 1.)
    #     beta = loss_config.get('beta', 1.0)
    #     alpha_focal = loss_config.get('alpha_focal', 0.25)
    #     gamma = loss_config.get('gamma', 2.0)
    #     return FocalDiceLoss(alpha = alpha, beta = beta, alpha_focal = alpha_focal, gamma = gamma)     
